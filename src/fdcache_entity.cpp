...
//key file location
#define keyLocation "/home/anush/Documents/key.txt"
#define tempLocation "/home/anush/Documents/temp.txt"

//function to grab key from key.txt. Return number of characters
void grabKey(int file, char *key, int size){
  int i=0;
  char buf[size];
  while (read(file, (char *)&buf[i++], 1)); //read key into buf
  for (int i=0; i<size; i++){
    key[i] = (char)buf[i]; //read buf into key var
  }
  close (file); //close file
}

void runRC4(int inFile, char *type){
  unsigned char transformed_key[NUM_OF_KEYS];
  RC4_KEY key;
  unsigned char inBuff[READ_WRITE_SIZE];
  unsigned char outBuff[READ_WRITE_SIZE];
  unsigned char salt[NUM_OF_SALT];
  const char *e = (char *)"e";
  const char *d = (char *)"d";

  lseek(inFile, 0, SEEK_SET); //make sure were reading from beginning of file
  int outFile = open(tempLocation, O_CREAT | O_RDWR, 0664); //rw-rw-r-- temp file to read file into during upload. allows us to add saltstring to inFile.

  //if encrypting
  if (strcmp(type, e) == 0){
    char saltBuf[SALT_STR_LEN];
    memset(saltBuf, 0, SALT_STR_LEN); //set saltBuf to all zeros before reading into it
    ...
    write(outFile, &saltBuf, SALT_STR_LEN); //write saltBuf to output file
  }
  //if decrypting
  else if (strcmp(type, d) == 0){
    lseek(inFile, 8, SEEK_SET); //read/write file offset to read saltBuf
    read(inFile, salt, NUM_OF_SALT); //read salt chars into salt
  }
  //now that we have the salt generated or read, we can create key with salt
  int size = 0;
  char *regular_key;
  //if file cannot be found, set key to default: fi1942 of size 6.
  if (access(keyLocation, F_OK) != 0){
    printf("Error: Could not find key.txt. Using default key instead.\n");
    size = 6;
    regular_key = (char *)malloc(size * sizeof(unsigned char *));
    sprintf(regular_key, "fi1942"); //set key to default: fi1942
  }
  //if file does exist read key from it
  else{
  int file = open(keyLocation, O_RDONLY); //try to open key file
  size = lseek(file, 0, SEEK_END) - 1; //scan key file to check for size
  lseek(file, 0, SEEK_SET); //reset key file pointer back to beginning
  regular_key = (char *)malloc(size * sizeof(unsigned char *));
  grabKey(file, regular_key, size); //grab key from key.txt
  }

  //generate key with salt and regular_key
  if (! EVP_BytesToKey(EVP_rc4(), EVP_sha256(),
  salt, (unsigned char *)regular_key, size, 1, transformed_key, NULL) )
  {
    printf("Error: Could not create encryption key.\n"); //if BytesToKey fails.
    exit(-1);
  }
  RC4_set_key(&key, NUM_OF_KEYS, transformed_key); //using transformed_key hash to create RC4_KEY key.

  //write data to outFile. using while loop to only enncrypt & write by READ_WRITE_SIZE bytes at a time
  ssize_t readBuff = 0; //counting bytes so program knows what buffer is if buffer < READ_WRITE_SIZE

  if (strcmp(type, e) == 0){
    while (readBuff = read(inFile, &inBuff, READ_WRITE_SIZE)){
      RC4(&key, readBuff, inBuff, outBuff); //encrypt inBuff to outBuff
      write(outFile, &outBuff, readBuff);
    }
    //after writing inFile to outFile, we now have an outFile of saltBuf + inputfile.
    lseek(outFile, 0, SEEK_SET); //reset read pointer
    lseek(inFile, 0, SEEK_SET); //reset read pointer
    while (readBuff = read(outFile, &outBuff, READ_WRITE_SIZE)){ //write outFile with saltstring back to inFile
      write(inFile, &outBuff, readBuff);
    }
  }

  else if (strcmp(type, d) == 0){
    int offset = 0;
    while (readBuff = read(inFile, &inBuff, 16)){ //16 = max size we can have to overwrite as were reading
	//(size of saltBuf). This allows us to continually read and write from same file.
      RC4(&key, readBuff, inBuff, outBuff); //encrypt inBuff to outBuff
      pwrite(inFile, &outBuff, readBuff, offset);
      offset += readBuff;
    }
    ftruncate(inFile, offset); //truncate file size to remove unneeded 16 bytes from deleted saltedString.
  }
  remove(tempLocation); //delete temp.txt file we created earlier
}
...
int FdEntity::Load(off_t start, off_t size, bool lock_already_held, bool is_modified_flag)
{
    ...
    fprintf(stdout, "[ANUSH LOGs] DECRYPTING");
    runRC4(fd, (char *)"d");
...
int FdEntity::RowFlush(const char* tpath, bool force_sync)
{
    ...
    if(!force_sync && !pagelist.IsModified()){
        ...
        fprintf(stdout, "[ANUSH LOGs] ENCRYPTING");
    runRC4(fd, (char *)"e");
...
ssize_t FdEntity::Write(const char* bytes, off_t start, size_t size)
{
    ...
    fprintf(stdout, "[ANUSH LOGs] ENCRPYTING");
    runRC4(fd, (char *)"e");
